    ...
    <div class="container mx-auto px-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                <?php foreach($produits as $produit): ?>
                    <div class="group bg-white rounded-3xl border border-slate-100 p-4 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300">
                        <div class="aspect-square bg-slate-50 rounded-2xl overflow-hidden mb-4 flex items-center justify-center">
                             <i class="fa-solid fa-microchip text-5xl text-slate-200"></i>
                        </div>
                        <span class="text-[10px] font-black uppercase text-blue-600"><?= htmlspecialchars($produit->getCategorie() ?? 'Tech') ?></span>
                        <h3 class="font-bold text-slate-800"><?= htmlspecialchars($produit->getNom()) ?></h3>
                        <div class="mt-4 flex justify-between items-center">
                            <span class="text-xl font-black text-slate-900"><?= number_format($produit->getPrix(), 2) ?> €</span>
                            <button class="w-10 h-10 bg-slate-900 text-white rounded-xl hover:bg-blue-600 transition">
